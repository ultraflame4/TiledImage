...
def generate_tiledimage_video(reference_video: Video, tiles: np.ndarray, tile_shape: tuple[int], useCuda=False,
    ...
    print(f"Will resize frames by a factor of {resize_factor}")
    ...
    for frame in reference_video.getFrame():
        ...
        print(
            f"{colorama.Fore.CYAN}>>>> Processing {reference_video.path} Frame [{index}/{reference_video.TotalFramesCount}] >>>>{colorama.Fore.RESET}")
        save_filepath = Path(f"./build/frames/{reference_video.path.name}_{index}.png")
        if save_filepath.exists():
            print(
                f"{colorama.Fore.YELLOW}>>>> Frame [{index}/{reference_video.TotalFramesCount}] already exits at {save_filepath}! Skipping... >>>>{colorama.Fore.RESET}")
            index += 1
            continue
        ...
        index += 1
        print(
            f"{colorama.Fore.GREEN}>>>>Finished and Saved Frame {index}/{reference_video.TotalFramesCount} in {timer.getTimeSinceLast()}s>>>>{colorama.Fore.RESET}")
...
def video_cli(
    ...
    TiledImage.video.generate_tiledimage_video(video, tiles, tile_shape, useCuda=useCuda, resize_factor=resize_factor)
